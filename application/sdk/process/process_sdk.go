...
var (
	process_sdk    *fabsdk.FabricSDK
	conf_path      string = "../application/conf/sdk_conf/processOrg/process-conf.yaml"
	channel_id     string = "processchannel"
	chaincode_name string = "processcc"
	peer_domain    string = "peer0.process.huniuping.com"
	peer_domain1    string = "peer0.supervise.huniuping.com"
	org            string = "ProcessOrg"
	user           string = "Admin"
	err            error
)

func init() {
	process_sdk, err = fabsdk.New(config.FromFile(conf_path))
	if err != nil {
...
func ChannelExecute(chaincode_name, execute_fcn string, args [][]byte) (channel.Response, error) {
	ctx := process_sdk.ChannelContext(channel_id, fabsdk.WithOrg(org), fabsdk.WithUser(user))
