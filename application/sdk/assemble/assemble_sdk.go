...
var (
	assemble_sdk   *fabsdk.FabricSDK
	conf_path      string = "../application/conf/sdk_conf/assembleOrg/assemble-conf.yaml"
	channel_id     string = "assemblechannel"
	chaincode_name string = "assemblecc"
	peer_domain    string = "peer0.assemble.huniuping.com"
	peer_domain1    string = "peer0.supervise.huniuping.com"
	org            string = "AssembleOrg"
	user           string = "Admin"
	err            error
)

func init() {
	assemble_sdk, err = fabsdk.New(config.FromFile(conf_path))
	if err != nil {
...
func ChannelExecute(chaincode_name, execute_fcn string, args [][]byte) (channel.Response, error) {
	ctx := assemble_sdk.ChannelContext(channel_id, fabsdk.WithOrg(org), fabsdk.WithUser(user))
