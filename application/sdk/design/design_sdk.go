...
var (
	design_sdk     *fabsdk.FabricSDK
	conf_path      string = "../application/conf/sdk_conf/designOrg/design-conf.yaml"
	channel_id     string = "designchannel"
	chaincode_name string = "drawingcc"
	peer_domain    string = "peer0.design.huniuping.com"
	peer_domain1    string = "peer0.supervise.huniuping.com"
	org            string = "DesignOrg"
	user           string = "Admin"
	err            error
)

func init() {
	design_sdk, err = fabsdk.New(config.FromFile(conf_path))
	if err != nil {
...
func ChannelExecute(chaincode_name, execute_fcn string, args [][]byte) (channel.Response, error) {
	ctx := design_sdk.ChannelContext(channel_id, fabsdk.WithOrg(org), fabsdk.WithUser(user))
