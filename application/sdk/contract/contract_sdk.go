...
var (
	contract_sdk   *fabsdk.FabricSDK
	conf_path      string = "../application/conf/sdk_conf/contractOrg/contract-conf.yaml"
	channel_id     string = "contractchannel"
	chaincode_name string = "contractcc"
	peer_domain    string = "peer0.contract.huniuping.com"
	peer_domain1    string = "peer0.supervise.huniuping.com"
	org            string = "ContractOrg"
	user           string = "Admin"
	err            error
)

func init() {
	contract_sdk, err = fabsdk.New(config.FromFile(conf_path))
	if err != nil {
...
func ChannelExecute(chaincode_name, execute_fcn string, args [][]byte) (channel.Response, error) {
	ctx := contract_sdk.ChannelContext(channel_id, fabsdk.WithOrg(org), fabsdk.WithUser(user))
